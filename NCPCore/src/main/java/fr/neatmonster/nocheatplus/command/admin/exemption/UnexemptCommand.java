...
public class UnexemptCommand extends BaseCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage((sender instanceof Player ? TAG : CTAG) + "请指定一个玩家 to unexempt.");
            return true;
        }
        else if (args.length > 3) {
            sender.sendMessage((sender instanceof Player ? TAG : CTAG) + "用法: /ncp unexempt (玩家) (检测类型).");
            return true;
        }
        ...
        if (playerName.equals("*")){
            ...
            sender.sendMessage((sender instanceof Player ? TAG : CTAG) + "移除所有玩家的绕过项目: " + c3 +""+ checkType);
            return true;
        }
        // Find player.
        ...
        if (id == null) {
            ...
            sender.sendMessage((sender instanceof Player ? TAG : CTAG) + "移除玩家 " + c3 +""+ playerName + c1 + " 对: " + c3 +""+ checkType + c1 + " 类型的检测");
        }
        return true;
    }
